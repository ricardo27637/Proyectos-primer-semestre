Proceso R_QUINONES_24041218
	
	Definir ganados, empatados, perdidos,totalP,totpuntos,maxp1,puntua1,puntua2,YA Como Entero;
	...
	totalP<-(ganados+empatados+perdidos)*3;
	YA<-ganados+empatados+perdidos;
	totpuntos<-ganados+empatados;
	maxp1<-(ganados*3)+(empatados*1);
	puntua1<-ganados*3;
	puntua2<-puntua1+empatados;
	porc1<-(maxp1*100)/(totalP);
	...
	Escribir "total de partidos jugados ",YA;
